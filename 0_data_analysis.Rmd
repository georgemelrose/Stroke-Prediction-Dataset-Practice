...
```{r cm for training data}
# Confusion Matrix
cm <- table(stroke_training_numeric$stroke_yn, knn_model)
cm
```
...
```
```{r}
roc(stroke_training$stroke_yn, attributes(knn_model)$prob)

plot(roc(stroke_training$stroke_yn, attributes(knn_model)$prob),
     print.thres = T,
     print.auc=T)
```
