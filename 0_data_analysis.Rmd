...
```{r creating a survival object, echo=FALSE}
...
```{r survival curve for smoking status, out.width=10, out.height=10}
ggsurvplot(
  fit_smoking,
  xlim = c(0, 5),
  ylim = c(0.7, 1),
...
```
```{r}
stroke_data$age_group_collapsed <- ifelse(stroke_data$age <= 60, "≤60", ">60")

# Convert to factor and set ≤60 as the base level
stroke_data$age_group_collapsed <- factor(stroke_data$age_group_collapsed, levels = c("≤60", ">60"))

# Verify the levels
levels(stroke_data$age_group_collapsed)


coxph(Surv(follow_up_years, stroke) ~ age_group_collapsed, data = stroke_data) %>% 
  tbl_regression(exp = TRUE) 
```
