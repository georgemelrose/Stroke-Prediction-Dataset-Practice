...
ggsurvplot(
  ...
  tables.theme = theme_minimal() + theme(
    ...
    # Adjust y-axis text in risk table
    pval.coord = c(0, 0.76), pval.method = TRUE, pval.method.coord = c(0, 0.75),
    legend.labs = legend_labels,
           legend.title = "Smoking Status",pval.size = 10, pval.method.size = 10 # Set legend title
  )
)
```

```{r}
coxph(Surv(follow_up_years, stroke) ~ age_category, data = stroke_data) %>% 
  tbl_regression(exp = TRUE) 
```
