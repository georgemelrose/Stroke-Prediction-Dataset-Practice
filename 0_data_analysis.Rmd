...
#removing the 3 NAs from the work type variable#
...
```{r adding back in underweight bmi category before SMOTE}

# Recode NA values to 'Underweight' in bmi_category
stroke_data$bmi_category <- factor(stroke_data$bmi_category)  # Ensure bmi_category is a factor

# Re-add 'Underweight' as a factor level (if it was removed during any processing)
stroke_data$bmi_category <- factor(stroke_data$bmi_category, levels = c(levels(stroke_data$bmi_category), "Underweight"))


stroke_data$bmi_category[is.na(stroke_data$bmi_category)] <- "Underweight"

```

```{r adding unknown as a factor level to work type variable}
# Add 'Unknown' as a factor level to work_type to make it suitable for SMOTE#

stroke_data$work_type <- factor(stroke_data$work_type, levels = c(levels(stroke_data$work_type), "Unknown"))

stroke_data$work_type[is.na(stroke_data$work_type)] <- "Unknown"
```
...
X <- stroke_data[, !names(stroke_data) %in% "stroke_yn"]  # Exclude target variable
...
target <- stroke_data$stroke_yn  # The target variable (should already be numeric or a factor)
...
# Step 1: Separate original 'Yes' cases (to retain them)
stroke_yes <- stroke_data %>% filter(stroke_yn == "Yes")
stroke_no <- stroke_data %>% filter(stroke_yn == "No")

# Step 2: Apply SMOTE only to generate new 'Yes' cases
X_no_stroke <- stroke_no[, !names(stroke_no) %in% "stroke_yn"]  # Features for 'No' cases
target_no_stroke <- stroke_no$stroke_yn  # Target variable for 'No' cases

# Apply SMOTE to generate synthetic 'Yes' cases
smote_result <- SMOTE(X_no_stroke, target_no_stroke, K = 5, dup_size = 70)  # Generates new 'Yes' cases

# Convert to dataframe and rename last column to stroke_yn
smote_data <- as.data.frame(smote_result$data)
colnames(smote_data)[ncol(smote_data)] <- "stroke_yn"

# Step 3: Retain original 'Yes' cases and undersample 'No' cases to keep dataset at 4,253 rows
num_new_yes <- round(0.085 * 4253)  # 8.5% of total dataset size
num_keep_no <- 4253 - num_new_yes - nrow(stroke_yes)  # Remaining 'No' cases to keep

# Randomly sample 'No' cases
stroke_no_sampled <- stroke_no %>% sample_n(num_keep_no)

# Randomly sample synthetic 'Yes' cases
smote_yes_sampled <- smote_data %>% filter(stroke_yn == "Yes") %>% sample_n(num_new_yes)

# Step 4: Merge everything back together
balanced_stroke_data <- bind_rows(stroke_yes, stroke_no_sampled, smote_yes_sampled)

# Step 5: Check final distribution
table(balanced_stroke_data$stroke_yn)
```
...
colnames(oversampled_data)[ncol(oversampled_data)] <- "stroke_yn"  # Rename target column

# Separate 'Yes' and 'No' cases
stroke_yes <- oversampled_data[oversampled_data$stroke_yn == "Yes", ]
stroke_no <- oversampled_data[oversampled_data$stroke_yn == "No", ]
...
# Check distribution
table(balanced_data$stroke_yn)

```

```{r}
# The balanced_data object contains both synthetic and original instances in 'data' 
# Access the 'data' part of the SMOTE result
balanced_data_df <- balanced_data$data

# Check if it's now a dataframe
str(balanced_data_df)

# View the first few rows of the balanced data
head(balanced_data_df)

table(balanced_data_df$class)
```
...
# Create a RShiny UI
shinyUI(
  fluidPage(theme = shinytheme("superhero"), padding=5,
  titlePanel("Stroke Prediction Application"), 
  # Create a side-bar layout
  sidebarLayout(
    # Create a side bar to for user input
   
    sidebarPanel(
      h4("Fill in Your details and click on Submit for a Prediction"),
      actionButton("predictButton", "Submit for a Prediction"),
      selectInput(inputId="gender", "Select Gender", choices = c("Female", "Male"), selected = "Male"),
      sliderInput("age", "Select Your Age:",
                  min = 18, max = 82,
                  value = 40),
      selectInput(inputId="hypertension_yn", "Do you have hypertension?", choices = c("No", "Yes"), selected = "No"),
      selectInput(inputId="heart_disease_yn", "Do you have heart disease?", choices = c("No", "Yes"), selected = "No"),
      selectInput(inputId="ever_married", "Are You Married?", choices = c("No","Yes"), selected = "No"),
      ...
      sliderInput("bmi", "Your BMI:",
                  min = 10, max = 98,
                  value = 30),
      selectInput(inputId="smoking_status", "What is your current Smoking Status?", choices = c("formerly.smoked", "never.smoked", "smokes", "Unknown"), selected = "never.smoked"
                  )),
    mainPanel(
