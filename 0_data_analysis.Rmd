...
-   There is more missingness for the smoking_status variable in those
    patients who have hypertension than those who do not.
...
showing missingness is run again. 
...
The above is a scatter plot of original and imputed data with the
magenta points being the imputed data points and the blue points being
the observed values. The matching shape is a good verification of the
imputed values indeed being plausible.
...
stroke_yes_post_smote <- balanced_data[balanced_data$stroke_yn == "Yes", ]
...
```{r}
# Step 1: Check for duplicates in the IDs
post_smote_stroke_ids <- unique(post_smote_stroke_ids)  # Remove duplicates from post_SMOTE IDs
original_stroke_ids <- unique(original_stroke_ids)  # Remove duplicates from original IDs

# Create a new dataframe for new stroke cases
new_stroke_cases_df <- data.frame(
  id = new_stroke_case_IDs, 
  stroke_yn = "Yes"  # Set stroke_yn to "Yes" for the new stroke cases
)

# Merge new stroke cases with original stroke data
stroke_data_updated <- stroke_data %>%
  left_join(new_stroke_cases_df, by = "id", suffix = c("", "_new")) %>%
  mutate(stroke_yn = if_else(!is.na(stroke_yn_new), "Yes", stroke_yn)) %>%
  select(-stroke_yn_new)  # Drop the temporary column
